...
func calculateBudget(area float64, construction ConstructionType, wg *sync.WaitGroup, ch chan<- Budget) {
	defer wg.Done()
	budget := area * construction.rate
	mu.Lock()
	ch <- Budget{construction.name, budget}
	mu.Unlock()
}
...
func main() {
	...
	ch := make(chan Budget, 3) // Buffered channel to collect budget results

	switch choice {
	case 1:
		wg.Add(1)
		go calculateBudget(area, house, &wg, ch)
	case 2:
		wg.Add(1)
		go calculateBudget(area, flats, &wg, ch)
	case 3:
		wg.Add(1)
		go calculateBudget(area, commercial, &wg, ch)
	case 4:
		wg.Add(3)
		go printHouse(&wg)
		go printFlats(&wg)
